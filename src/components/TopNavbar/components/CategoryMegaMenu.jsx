import { getMegaMenuItems } from '@/helpers/menu';
import Image from 'next/image';
import React from 'react';
import { Button, Card, CardBody, CardTitle, Col, Row } from 'react-bootstrap';
import { FaEllipsis } from 'react-icons/fa6';
const CategoryMegaMenu = () => {
  const CategoryMegaMenuData = getMegaMenuItems();
  return <li className="dropdown-submenu dropend">
      <a className="dropdown-item dropdown-toggle arrow-none" href="#">Marketing <span className='float-end'> <FaEllipsis size={14} /></span></a>
      <div className="dropdown-menu dropdown-menu-start dropdown-width-lg" data-bs-popper="none">
        <Row className="p-4">
          <Col xl={6} xxl={4} className="mb-4 mb-xl-0">
            <h6 className="mb-0">Get started</h6>
            <hr />
            <ul className="list-unstyled">
              {CategoryMegaMenuData.getStarted.map((item, idx) => <li key={idx}> <a className="dropdown-item" href="#">{item.label}</a> </li>)}
            </ul>
          </Col>
          <Col xl={6} xxl={4} className="mb-4 mb-xl-0">
            <h6 className="mb-0">Degree</h6>
            <hr />
            {CategoryMegaMenuData.degree.map((degree, idx) => <div className="d-flex mb-4 position-relative" key={idx}>
                {degree.image && <Image src={degree.image} className="icon-md" alt='uniLogo' />}
                <div className="ms-3">
                  <a className="stretched-link h6 mb-0" href="#">{degree.label}</a>
                  <p className="mb-0 small">{degree.description}</p>
                </div>
              </div>)}
          </Col>
          <Col xl={6} xxl={4}>
            ...
            <hr />
            {CategoryMegaMenuData.certificate.map((certificate, idx) => {
            const Icon = certificate.icon;
            return <div className="d-flex mb-4 position-relative" key={idx}>
                  {Icon && <h2 className="mb-0"><Icon className={`${certificate.iconClass}`} /></h2>}
                  <div className="ms-2">
        ...
        <Row>
          <Col xs={12}>
            <Card className="bg-blue rounded-0 rounded-bottom p-3 position-relative overflow-hidden" style={{
            ...
            backgroundSize: 'cover'
              ...
              <CardBody>
                ...
                <Button variant='dark' size='sm' className="mb-0">Purchase Premium</Button>
              </CardBody>
            </Card>
          </Col>
        </Row>
