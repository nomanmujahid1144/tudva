...
// src/services/matrixService.js
// ✅ FINAL VERSION - No polling, Matrix only for history/persistence
...
class MatrixService {
  ...
  /**
   * Join room - ONLY fetches initial message history
   */
  async joinRoom(roomId, userCredentials) {
    try {
      console.log('🔄 Joining Matrix room (for history only):', roomId);
      ...
      console.log('✅ Joined Matrix room, fetched', result.data.messages?.length || 0, 'history messages');
  ...
  /**
   * Send message to Matrix (for persistence)
   */
  async sendMessage(content, msgType = 'm.text') {
    if (!content.trim() || !this.currentRoomId) {
      return { success: false, error: 'No content or room' };
    ...
    try {
      ...
      console.log('✅ Message saved to Matrix');
      return { success: true, eventId: result.data.eventId };
