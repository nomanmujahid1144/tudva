...
const CourseListCard = ({
  ...
  return <tr>
      <td className="d-flex align-items-center position-relative">
        <div className="w-60px"><Image src={image} className="rounded" alt='avatar' /></div>
        <h6 className="table-responsive-title mb-0 ms-2"><a href="#" className="stretched-link">{title}</a></h6>
      </td>
      <td>{enrolled}</td>
      <td>
        <span className={`badge bg-${status === 'Live' ? 'success' : 'warning'} bg-opacity-15 text-${status === 'Live' ? 'success' : 'warning'}`}>{status}</span>
      </td>
      <td>
        <Button variant='info-soft' size='sm' className="mb-0">View</Button>
      </td>
    </tr>;
};
const CoursesList = () => {
        ...
        <CardBody className="pb-0">
          <div className="table-responsive border-0">
            <table className="table table-dark-gray align-middle p-4 mb-0 table-hover">
              ...
              <tbody>
                {coursesData.slice(0, 5).map((item, idx) => <CourseListCard {...item} key={idx} />)}
              </tbody>
            </table>
          </div>
        </CardBody>
