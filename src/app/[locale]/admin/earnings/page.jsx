...
const InvoiceHistoryCard = ({
  ...
  return <tr>
      <td>{id}</td>
      <td>
        <h6 className="table-responsive-title mb-0"><a href="#">{name}</a></h6>
      </td>
      ...
      <td>
        <Image src={paymentMethod.image} height={paymentMethod.type === 'paypal' ? 21 : 50} alt="paymentMethodImg" />
      </td>
      <td>${amount}&nbsp;
      ...
      <td>
        <div className={`badge bg-${status === 'paid' ? 'success' : status === 'pending' ? 'orange' : 'danger'} bg-opacity-10 text-${status === 'paid' ? 'success' : status === 'pending' ? 'orange' : 'danger'}`}>{status}</div>
      </td>
      <td>
        <a href="#" className="btn btn-primary-soft btn-round me-1 mb-1 mb-md-0" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Download" aria-label="Download">
          <BsDownload />
        </a>
      </td>
    </tr>;
