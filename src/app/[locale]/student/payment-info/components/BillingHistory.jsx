...
const BillingHistoryData = ({
  ...
  return <tr>
      ...
      <td>
        <h6 className="mt-2 mt-lg-0 mb-0"><a href="#">{name}</a></h6>
      </td>
      <td>
        {paymentMethod.number ? <Image src={paymentMethod.image} height={40} width={40} className="h-40px" alt='image' /> : <Image src={paymentMethod.image} height={21} width={80} className="w-80px" alt='image' />}
        <span className="ms-2">{paymentMethod.number}</span></td>
      <td>
        <span className={`badge ${status === 'paid' ? 'bg-success text-success' : status === 'pending' ? 'bg-orange text-orange' : 'bg-danger text-danger'}  bg-opacity-10 `}>{status}</span>
      </td>
      <td>{currency}{price}</td>
      <td>
        <Button variant='primary-soft' className="btn-round me-1 mb-1 mb-md-0" data-bs-toggle="tooltip" data-bs-placement="top" title="Download"><BsDownload /></Button>
      </td>
    </tr>;
};
