...
const TopicsCard = ({
  ...
  return <Card className="bg-light h-100">
      ...
      <CardBody>
        <ul className="nav flex-column">
          {features.map((data, idx) => <li className="nav-item" key={idx}><Link className="nav-link d-flex" href="/help/center-detail"><FaAngleRight size={18} className="text-primary pt-1 me-2" />{data}</Link></li>)}
        </ul>
      </CardBody>
    </Card>;
};
const RecommendedTopics = () => {
  return <section>
      <Container>
        ...
        <Row className="g-4">
          {topicData.map((item, idx) => <Col md={6} xl={3} key={idx}>
                <TopicsCard {...item} />
              </Col>)}
        </Row>
      </Container>
