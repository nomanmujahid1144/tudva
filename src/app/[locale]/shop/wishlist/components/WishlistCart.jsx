  ...
  return <Card className="shadow">
      ...
      <CardBody className="pb-0">
        <div className="d-flex justify-content-between mb-2">
          ...
          <Link href="" className="text-danger" onClick={toggle}>{isOpen ? <FaHeart fill="red" /> : <FaRegHeart />}</Link>
        </div>
      ...
      <CardFooter className="pt-0 pb-3">
        ...
        <div className="d-flex justify-content-between ">
          <span className="h6 fw-light mb-0 icons-center "><FaRegClock className="text-danger me-2" />{duration}</span>
          <span className="h6 fw-light mb-0 icons-center "><FaTable className="text-orange me-2" />{lectures} lectures</span>
        </div>
      </CardFooter>
    </Card>;
};
  ...
  return <section className="pt-5">
      <Container>
        <div className="d-sm-flex justify-content-sm-between align-items-center mb-4">
          ...
          <div className="text-end"> <button className="btn btn-danger-soft mb-0"><FaTrash className="me-2" />Remove all</button> </div>
        </div>
        <Row className="g-4">
          {wishlistCartData?.slice(0, 4).map((item, idx) => <Col sm={6} lg={4} xl={3} key={idx}>
                <WishlistCard {...item} />
              </Col>)}
        </Row>
      </Container>
