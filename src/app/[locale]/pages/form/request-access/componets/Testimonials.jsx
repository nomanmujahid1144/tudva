...
const Testimonials = () => {
  ...
  const testimanialsSliderSettings = {
    arrowKeys: true,
    gutter: 20,
          ...
          <Col xs={10} className="mx-auto">
            <Row className="g-4">
              <div className="tiny-slider arrow-round arrow-dark arrow-hover">
                {allTestimonials && <TinySlider settings={testimanialsSliderSettings}>
                    {allTestimonials.map((item, idx) => <Col xs={6} key={idx}>
                        <p className="fs-5">{item.description}</p>
                        <div className="d-flex align-items-center">
                          <div className="avatar avatar-md">
                            {item.course?.avatar && <Image className="avatar-img rounded-circle" src={item.course.avatar} alt="avatar" />}
                          </div>
                          <div className="ms-2">
