      ...
      <div className="position-relative">
        ...
        <div className="card-img-overlay d-flex align-items-start flex-column p-3">
          <div className="w-100 mb-auto d-flex justify-content-end" onClick={toggle}>
            <span role="button" className="icon-sm bg-white rounded-2">{isTrue ? <FaHeart fill="red" /> : <FaRegHeart className="text-danger" />}</span>
          </div>
        ...
        <Row>
          <div className="tiny-slider arrow-round arrow-creative arrow-blur">
            <TinySlider settings={eventSliderSettings}>
              {allEvents?.map((event, idx) => <EventCard key={idx} event={event} />)}
            </TinySlider>
          </div>
        </Row>
      </Container>
