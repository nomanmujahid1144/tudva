  ...
  return <Card className="bg-transparent">
      <div className="position-relative">
        ...
        <div className="card-img-overlay d-flex align-items-start flex-column p-3">
          <div className="w-100 mt-auto">
            {hasLive ? <Link href="" className="badge text-success bg-white fs-6 rounded-1"><FaCircle className="text-success me-2" />Live</Link> : <Link href="" className="badge text-bg-white fs-6 rounded-1"><FaRegCalendarAlt className="text-orange me-2" />{date}</Link>}
          </div>
        </div>
      </div>
      <CardBody className="px-2">
        ...
        <div className="d-flex justify-content-between align-items-center">
          <address className="mb-0"><FaMapMarkerAlt className="me-2" />{city}</address>
          <Button variant="primary-soft" size="sm" className="mb-0">Join now</Button>
        </div>
      </CardBody>
    </Card>;
};
const EducationEvents = () => {
  ...
  return <section className="pb-0 pb-md-5">
      <Container>
        <Row className="mb-4">
          <h2 className="mb-0">Upcoming <span className="text-warning">Education</span> Events</h2>
        </Row>
        <Row>
          <div className="tiny-slider arrow-round arrow-creative arrow-blur arrow-hover">
            {allEvents && <TinySlider settings={eventSliderSettings}>
                {allEvents?.map((event, idx) => <EventCard event={event} key={idx} />)}
              </TinySlider>}
          </div>
        </Row>
      </Container>
