...
const FeatureCourseCard = ({
  ...
  return <Card className="p-2 shadow h-100">
      ...
      <CardBody className="px-2">
        ...
        <hr />
        <CardTitle as='h6'><Link href="">{title}</Link></CardTitle>
        <div className="d-flex justify-content-between align-items-center mb-0">
          <div><a href="#" className="badge bg-info bg-opacity-10 text-info me-2"><FaCircle className="small fw-bold" /> {label} </a></div>
          <h5 className="text-success mb-0">{currency}{price}</h5>
        </div>
      </CardBody>
    </Card>;
};
const FeaturedCourses = async () => {
  const allFeaturedCourses = await getAllCourses();
  return <section className="pt-0 pt-md-5">
      <Container>
        ...
        <Row className="g-4">
          {allFeaturedCourses.slice(0, 8).map((course, idx) => <Col md={6} lg={4} xxl={3} key={idx}>
              <FeatureCourseCard course={course} />
            </Col>)}
        </Row>
        <div className="text-center mt-5">
          <Button variant="primary-soft">View more<FaSync className="ms-2" /></Button>
        </div>
      </Container>
    </section>;
};
