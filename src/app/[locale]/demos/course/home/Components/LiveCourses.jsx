...
import { FaChevronLeft, FaChevronRight, FaCircle, FaPlay } from "react-icons/fa";
...
const students = [avatar1, avatar2, avatar3, avatar6];
const CourseCard = ({
  course
}) => {
  const {
  ...
  return <Card className="p-2">
      ...
      <CardBody>
        ...
        <div className="d-sm-flex justify-content-sm-between align-items-center mt-3">
          <div>
            <h6 className="mb-1 fw-normal"><FaCircle className="fw-bold text-success small me-2" />Live Students</h6>
            <ul className="avatar-group mb-2 mb-sm-0">
              {students.map((student, idx) => <li className="avatar avatar-xs" key={idx}>
          ...
          <Button size="sm" variant="success" className="mb-0">Join now</Button>
        </div>
      </CardBody>
    </Card>;
};
const LiveCourses = () => {
  const allLiveCourses = useFetchData(getAllCourses);
  ...
  return <section className="bg-light position-relative">
      ...
      <Container>
        <Row className="g-4 align-items-center justify-content-between">
          <Col md={6} xl={4}>
            <h2 className="fs-1">Today&apos;s Top Free Live Courses</h2>
            ...
            <a href="#" className="btn btn-orange mb-0">Get premium courses</a>
          </Col>
          <Col md={6} xl={8}>
            <Row>
              <div className="tiny-slider arrow-round arrow-blur">
                {liveCourses && <TinySlider settings={courseSliderSettings} data-autoplay="false" data-edge={2} data-arrow="true" data-dots="false" data-items-lg={1} data-items-xl={2}>
