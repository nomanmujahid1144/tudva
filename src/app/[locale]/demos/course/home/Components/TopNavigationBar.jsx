  ...
  return <TopNavbar className="navbar-transparent">
      <Container>
        ...
        <button className="navbar-toggler ms-auto" type="button" data-bs-toggle="collapse" data-bs-target="#navbarCollapse" aria-controls="navbarCollapse" aria-expanded="true" aria-label="Toggle navigation">
          <span className="me-2"><FaSearch className="fs-5" /></span>
        </button>
        ...
        {CATEGORY_MENU_ITEMS.map((item, idx) => {
          return <Fragment key={item.key + idx}>
                    {item.children ? <MenuItemWithChildren item={item} level={1} itemClassName="nav-item dropdown-menu-shadow-stacked" linkClassName={clsx('arrow-none nav-link rounded-3 text-primary px-3 py-3 py-xl-0')} /> : <MenuItem item={item} level={1} />}
        ...
        <div className="navbar-nav position-relative overflow-visible me-3">
          <Link href="#" className="nav-link">	<FaShoppingCart className="fs-5" /></Link>
          <span className="position-absolute top-0 start-100 translate-middle badge rounded-circle bg-success mt-xl-2 ms-n1">5
            <span className="visually-hidden">unread messages</span>
          </span>
        </div>
        <ul className="navbar-nav navbar-nav-scroll me-3 d-none d-xl-block">
          <Dropdown className="nav-item">
            <DropdownToggle as="a" className="nav-link dropdown-toggle arrow-none" href="#" id="language" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
              <FaGlobe className="me-2" />
              <span className="d-none d-lg-inline-block">Language</span><FaChevronDown className="ms-1" size={10} />
            </DropdownToggle>
            <ul className="dropdown-menu dropdown-menu-end min-w-auto" aria-labelledby="language">
              <li><DropdownItem href="#"><Image alt="uk" height={12} width={18} className="fa-fw me-2" src={ukFlag} />English</DropdownItem></li>
              <li><DropdownItem href="#"><Image alt="gr" height={12} width={18} className="fa-fw me-2" src={grFlag} />German </DropdownItem></li>
              <li><DropdownItem href="#"><Image alt="sp" height={12} width={18} className="fa-fw me-2" src={spFlag} />French</DropdownItem></li>
            </ul>
          </Dropdown>
        </ul>
        <div className="navbar-nav d-none d-lg-inline-block">
          <button className="btn btn-danger-soft mb-0"><FaSignInAlt className="me-2" />Sign Up</button>
        </div>
      </Container>
