...
import { activeLogsIpData } from '../data';
const ActiveLogCard = ({
  browser,
  ip,
  time
  ...
  return <>
      ...
      <div className="bg-light rounded-3 p-4 mb-3">
        <div className="d-md-flex justify-content-between align-items-center">
          ...
          <div>
            <Button onClick={toggle} className="btn btn-primary mb-1" data-bs-toggle="modal" data-bs-target="#changePassword">Change Password</Button>
          ...
          <div className="table-responsive border-0">
            <table className="table table-dark-gray align-middle p-4 mb-0 table-hover">
              ...
              <tbody>
                ...
                {activeLogsIpData.map((item, idx) => <ActiveLogCard key={idx} {...item} />)}
              </tbody>
            </table>
          </div>
        </CardBody>
      ...
      <Modal onHide={toggle} show={isOpen} className="fade" id="changePassword" tabIndex={-1} aria-labelledby="changePasswordLabel" aria-hidden="true">
        <ModalHeader className="modal-header bg-dark">
          <h5 className="modal-title text-white" id="changePasswordLabel">Change Password</h5>
          <button onClick={toggle} type="button" className="btn btn-sm btn-light mb-0 ms-auto" data-bs-dismiss="modal" aria-label="Close"><BsXLg /></button>
        </ModalHeader>
        <ModalBody className="modal-body">
          <form className="row">
            ...
            <Col xs={12} className="mb-3">
              <label className="form-label">New Passowrd <span className="text-danger">*</span></label>
              <input type="password" className="form-control" placeholder="Enter new passowrd" />
            </Col>
            <Col xs={12}>
              <label className="form-label">Confirm Passowrd <span className="text-danger">*</span></label>
              <input type="password" className="form-control" placeholder="Enter confirm passowrd" />
            </Col>
          </form>
        </ModalBody>
        <ModalFooter>
          <button type="button" onClick={toggle} className="btn btn-danger-soft my-0" data-bs-dismiss="modal">Close</button>
          <button type="button" className="btn btn-success my-0">Change Password</button>
        </ModalFooter>
      </Modal>
