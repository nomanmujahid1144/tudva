...
const CourseCard = ({
  ...
  return <tr>
      <td>
        <div className="d-flex align-items-center position-relative">
          <div className="w-60px">
            <Image src={image} className="rounded" alt='image' />
          </div>
          <h6 className="table-responsive-title mb-0 ms-2">
            <Link href="#" className="stretched-link">{title}</Link>
          </h6>
        </div>
      </td>
      <td>
      ...
      <td> <span className={`badge text-bg-${badge.text === 'Beginner' ? 'primary' : badge.text === 'All level' ? 'orange' : 'purple'}`}>{badge.text}</span> </td>
      <td>{price}</td>
      <td> <span className={`badge text-${status === 'pending' ? 'warning' : 'success'} bg-${status === 'pending' ? 'warning' : 'success'} bg-opacity-15 `}>{status}</span> </td>
      {status === 'pending' ? <td>
