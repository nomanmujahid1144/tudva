...
const CartCard = ({
  ...
  return <tr>
      <td>
        <div className="d-lg-flex align-items-center">
          <div className="w-100px w-md-80px mb-2 mb-md-0">
            <Image src={image} className="rounded" alt='courseImage' />
          </div>
      ...
      <td>
        <Button variant='success-soft' size='sm' className="px-2 me-1 mb-1 mb-md-0"><FaRegEdit size={15} /></Button>
        <button className="btn btn-sm btn-danger-soft px-2 mb-0"><FaTimes size={14} /></button>
      </td>
    </tr>;
};
const CartDetails = () => {
        ...
        <Row className="g-4 g-sm-5">
          <Col lg={8} className="mb-4 mb-sm-0">
            <Card className="card-body p-4 shadow">
              <Alert show={isTrue} onClose={toggle} className="alert alert-danger alert-dismissible d-flex justify-content-between align-items-center fade show py-3 pe-2" role="alert">
                <div>
                ...
                <button type="button" onClick={toggle} className="btn btn-link mb-0 text-primary-hover text-end" data-bs-dismiss="alert" aria-label="Close"><BsXLg></BsXLg></button>
              </Alert>
              <div className="table-responsive border-0 rounded-3">
                <table className="table align-middle p-4 mb-0">
                  <tbody className="border-top-0">
                    {cartData?.slice(0, 2).map((item, idx) => <CartCard key={idx} {...item} />)}
                  </tbody>
                </table>
              </div>
          ...
          <Col lg={4}>
            <Card className="card-body p-4 shadow">
              ...
              <div className="d-grid">
                <Link href='/shop/checkout' className="btn btn-lg btn-success">Proceed to Checkout</Link>
              </div>
