      ...
      <Card>
        <CardHeader className="border-bottom p-0 pb-3">
          ...
          <form className="row g-4">
            ...
            <Col sm={6} lg={3}>
              <Button variant="primary-soft" className="mb-0 w-100" onClick={toggle}>Create Post</Button>
            </Col>
          </form>
        </CardHeader>
        <CardBody className="p-0 pt-3">
          <div className="vstack gap-3">
            {discussData.map((item, idx) => <div className={clsx({
            "border-bottom": discussData.length - 1 != idx
          })} key={idx}>
                <div className="d-sm-flex justify-content-sm-between mb-3">
                  <div className="d-flex align-items-center">
                    ...
                    <div className="ms-2">
                      <h6 className="mb-0"><a href="#">{item.name}</a></h6>
                ...
                <ul className="nav nav-divider pb-3 small">
                  <li className="nav-item"> <Link className="text-primary-hover" href=""> <BsHandThumbsUp className="me-2" />Like (3)</Link> </li>
                  <li className="nav-item"> <Link className="text-primary-hover" href=""> <BsChatLeft className="me-2" />Reply</Link> </li>
                </ul>
              </div>)}
          </div>
        </CardBody>
      </Card>
      <Modal show={isOpen} onHide={toggle} className="fade" id="modalCreatePost" tabIndex={-1} aria-labelledby="modalLabelCreatePost" aria-hidden="true">
          <form onSubmit={handleSubmit(() => {})}>
        <ModalHeader closeButton>
          <h5 className="modal-title" id="modalLabelCreatePost">Create Post </h5>
        </ModalHeader>
        <ModalBody>
        ...
        <ModalFooter>
          <Button variant="success-soft" type="submit">Post</Button>
        </ModalFooter>
          </form>
      </Modal>
    </>;
