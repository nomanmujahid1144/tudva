...
const MemberCard = ({
  ...
  return <div className="text-center">
      ...
      <h6 className="mb-0"><Link href="">{member.name}</Link></h6>
      <p className="mb-0 small">{member.role}</p>
      <ul className="list-inline mb-0">
        {Array(Math.floor(member.rating.star)).fill(0).map((_star, idx) => <li key={idx} className="list-inline-item me-1 small"><FaStar size={14} className="text-warning" /></li>)}
        {!Number.isInteger(member.rating.star) && <li className="list-inline-item me-1 small"> <FaStarHalfAlt size={14} className="text-warning" /> </li>}
        {member.rating.star < 5 && Array(5 - Math.ceil(member.rating.star)).fill(0).map((_star, idx) => <li key={idx} className="list-inline-item me-1 small"><FaRegStar size={14} className="text-warning" /></li>)}
      </ul>
    </div>;
};
const Team = () => {
        ...
        <Row>
          ...
          <Col md={8}>
            <div className="d-sm-flex justify-content-sm-between">
              ...
              <a href="#" className="btn btn-light mt-2">Join Team</a>
            </div>
