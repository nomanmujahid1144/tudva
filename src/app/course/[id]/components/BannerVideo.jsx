...
const BannerVideo = ({ course, selectedVideo, onVideoSelect }) => {
  ...
  console.log("Selected video in BannerVideo:", selectedVideo);
  console.log("Course data in BannerVideo:", course);

  // Set default video if course data is available
  useEffect(() => {
    // If we already have a selected video, don't override it
    if (selectedVideo) {
      return;
    }
      ...
      if (moduleKeys.length > 0) {
        ...
        if (firstModuleVideos && firstModuleVideos.length > 0) {
          ...
          if (firstVideoWithUrl) {
            ...
            if (onVideoSelect) {
              onVideoSelect(firstVideoWithUrl);
            }
          }
        }
      }
    }
  }, [course, selectedVideo, onVideoSelect]);
      ...
      {width < 992 && (
        <Offcanvas show={isOpen} onHide={toggle} placement="end" className="offcanvas-lg" tabIndex={-1} id="offcanvasSidebar" aria-labelledby="offcanvasSidebarLabel">
          ...
          <OffcanvasBody className="p-3 p-lg-0">
            <Col xs={12}>
              <Playlist course={course} onVideoSelect={handleVideoSelect} selectedVideo={currentVideo} />
            </Col>
          </OffcanvasBody>
        </Offcanvas>
      )}
