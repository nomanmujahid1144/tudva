  ...
  return <Card className="shadow h-100">
      ...
      <CardBody className="pb-0">
        <div className="d-flex justify-content-between mb-2">
          <a href="#" className={clsx("badge bg-opacity-10", badge.class)}>{badge.text}</a>
          <span role="button" className="h6 mb-0" onClick={toggle}>{isOpen ? <FaHeart fill="red" /> : <FaRegHeart />}</span>
        </div>
        <CardTitle className="fw-normal"><Link href="">{title}</Link></CardTitle>
        <p className="mb-2 text-truncate-2">{description}</p>
        {rating && <ul className="list-inline mb-0">
...
const Courses = ({
  ...
  return <>
      <TabPane eventKey={courseKey} className="fade show" id="course-pills-tabs-1" role="tabpanel" aria-labelledby="course-pills-tab-1">
        <Row className="g-4">
          {filteredCourses?.map((course, idx) => <Col sm={6} lg={4} xl={3} key={idx}>
        ...
        <TabContainer defaultActiveKey='web-design'>
          <Nav className="nav-pills nav-pills-bg-soft justify-content-sm-center mb-4 px-3" id="course-pills-tab" role="tablist">
            <NavItem className="me-2 me-sm-5">
              <NavLink as='button' eventKey="web-design" className="mb-2 mb-md-0" type="button" role="tab">Web Design</NavLink>
            </NavItem>
            <NavItem className="me-2 me-sm-5">
              <NavLink as='button' eventKey="development" className="mb-2 mb-md-0" type="button" role="tab">Development</NavLink>
            </NavItem>
            <NavItem className="me-2 me-sm-5">
              <NavLink as='button' eventKey="graphic-design" className="mb-2 mb-md-0" type="button" role="tab">Graphic Design</NavLink>
            </NavItem>
            <NavItem className="me-2 me-sm-5">
              <NavLink as='button' eventKey="marketing" className="mb-2 mb-md-0" type="button" role="tab">Marketing</NavLink>
            </NavItem>
            <NavItem className="me-2 me-sm-5">
              <NavLink as='button' eventKey="finance" className="mb-2 mb-md-0" type="button" role="tab">Finance</NavLink>
            </NavItem>
          </Nav>
          <TabContent id="course-pills-tabContent">
            {categories.map((category, idx) => <Courses courseKey={category} key={idx} />)}
          </TabContent>
        </TabContainer>
      </Container>
    </section>;
