      ...
      <Container>
        <Row className="g-md-5 align-items-center">
          ...
          <Col lg={6} className="mt-5 mt-lg-0">
            <h2 className="mb-3">Welcome to WBS University</h2>
            <p className="mb-3 mb-lg-4">Two before narrow not relied on how except moment myself Dejection assurance Mrs led certainly So gate at no only none open Betrayed.</p>
            <Row className="g-4">
              {aboutData.map((about, idx) => {
              const Icon = about.icon;
              return <Col xs={6} key={idx}>
                    <div className="d-sm-flex align-items-center">
                      <div className={clsx("icon-md fs-5 text-white rounded", about.variant)}>
                        <Icon />
                      </div>
                      <div className="ms-0 ms-sm-3 mt-2 mt-sm-0">
                        <h6 className="mb-0">{about.title}</h6>
                        <div className="small"><BsStopwatch className="me-2" />{about.subTitle}</div>
                      </div>
                    </div>
                  </Col>;
            })}
            </Row>
            <a href="#" className="btn btn-primary-soft mb-0 mt-3 mt-lg-4">More about us</a>
          </Col>
        </Row>
      </Container>
    </section>;
