...
const TopCountries = () => {
  return <section className="position-relative overflow-hidden py-0 pt-xl-5">
      ...
      backgroundImage: `url(${mapSvg.src})`,
              ...
              {topCountries.map((country, idx) => <Col sm={6} md={4} key={idx}>
                  <Card className="card-body shadow p-4">
                    <div className="d-flex align-items-center">
                      <Image width={60} height={40} src={country.flag} className="h-40px" alt="country" />
                      <h5 className="ms-3 mb-0"> <Link href="/abroad/single" className="stretched-link">{country.name}</Link></h5>
                    </div>
                  </Card>
                </Col>)}
            </Row>
