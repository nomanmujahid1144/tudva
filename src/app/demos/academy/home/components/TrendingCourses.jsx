...
const CourseCard = ({
  ...
  return <Card className="shadow-hover overflow-hidden bg-transparent">
      <div className="position-relative">
        <Image className="card-img-top" src={studentImage} alt="Card image" />
          ...
          <div className="w-100 mt-auto d-inline-flex">
            <div className="d-flex align-items-center bg-white p-2 rounded-2">
              <div className="avatar avatar-sm me-2">
                <Image className="avatar-img rounded-1" src={avatar} alt="avatar" />
              </div>
              <div>
                <h6 className="mb-0"><Link href="" className="text-dark">{name}</Link></h6>
      ...
      <CardBody>
        <div className="d-flex justify-content-between mb-3">
          <div className="hstack gap-2">
            ...
            <a href="#" className="badge text-bg-dark">{courseDuration} month</a>
          </div>
          <a href="#" className="h6 fw-light mb-0"><FaRegBookmark /></a>
        </div>
        <CardTitle><a href="#">{title}</a></CardTitle>
        ...
        <div className="d-flex justify-content-between align-items-center mb-2">
          <h4 className="text-success mb-0">{currency}{price}</h4>
          <span className="h6 fw-light mb-0 me-3"><FaRegClock className="text-danger me-2" />{duration}</span>
        </div>
      </CardBody>
    </Card>;
};
const TrendingCourses = async () => {
  const trendingCourses = await getAllCourses();
  return <section className="pt-0 pt-lg-5">
      <Container>
        ...
        <Row className="g-4">
          {trendingCourses.slice(0, 3).map((course, idx) => <Col md={6} xl={4} key={idx}>
