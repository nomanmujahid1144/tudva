          ...
          <Col md={12}>
            <div className="bg-light rounded-3 p-4">
              <div className="tiny-slider arrow-round arrow-creative arrow-blur arrow-hover py-1">
                <TinySlider settings={courseSliderSettings} data-autoplay="true" data-gutter={80} data-arrow="true" data-dots="false" data-items={5} data-items-lg={3} data-items-md={2} data-items-xs={1}>
                  {courseImages.map((course, idx) => <div key={idx}>
                      <div className="bg-body text-center rounded-2 border py-2 px-1 position-relative">
                        <Image src={course.logo} className="h-40px" alt="course-name" />
                        <a href="#" className="text-primary-hover stretched-link"><span className="h6 ms-2">{course.name}</span></a>
                      </div>
                    </div>)}
                </TinySlider>
              </div>
